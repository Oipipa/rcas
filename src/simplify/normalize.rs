...
fn normalize_pow_form(base: Expr, exp: Expr, size_limit: usize) -> Expr {
    ...
    // Distribute over products for integer exponents (respect size limits).
...
fn distribute_pow_over_product(base: &Expr, exp: &Rational, size_limit: usize) -> Option<Expr> {
    let power = exp.to_integer().to_i64()?;
    let (mut const_factor, factors) = collect_product(base);
    if factors.is_empty() || (factors.len() == 1 && const_factor.is_one()) {
        return None;
    }

    if expr_size(base) * power.unsigned_abs() as usize > size_limit {
        return None;
    }
...
fn factor_pair(left: &Expr, right: &Expr, size_limit: usize) -> Option<Expr> {
    ...
    if expr_size(&factored) <= size_limit
        && expr_size(&factored) <= expr_size(&simplify_add(left.clone(), right.clone()))
    {
        Some(factored)
    } else {
        None
